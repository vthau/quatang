...
function addItem() {
    ...
    popup.find('.modal-header .modal-title').text("Tạo Quyền Truy Cập");
...
function editItem(id) {
    ...
    popup.find('.alert').addClass('hidden');
    popup.find('.modal-header .modal-title').text("Sửa Quyền Truy Cập");
...
function confirmUpdateItem() {
    ...
    if (!name || name === '') {
        popup.find('#req-title .alert').removeClass('hidden')
            .text("Hãy nhập tên quyền truy cập.");
        return false;
    }
