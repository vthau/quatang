...
function jspopupdelete() {
    var popup = jQuery("#modal_delete_item");
    jQuery.ajax({
        url: gks.baseURL + "/admin/order/delete",
        type: "post",
        data: {
...
function updateStatus(ele, id) {
    jQuery.ajax({
        url: gks.baseURL + "/admin/order/update-shipment",
        type: "post",
        data: {
            item_id: id,
            status: ele.value,
...
function confirmOrder(id, key, value) {
    ...
    if (key === "status") {
        message =
            "Bạn có chắc muốn xác nhận đơn hàng này đã CHƯA ĐƯỢC thanh toán không?";
        if (parseInt(value)) {
            message =
                "Bạn có chắc muốn xác nhận đơn hàng này ĐÃ ĐƯỢC thanh toán không?";
        }
    }
