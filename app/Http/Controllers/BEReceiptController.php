...
class BEReceiptController extends Controller
{
    ...
    {
        if (!$this->_viewer->isAllowed('order_view')) {
        ...
        $products = $select->select('user_carts.*')->paginate(20);
        ...
        //products
        $values['products'] = Product::select("id", "title")
            ->where('deleted', 0)
            ->orderByRaw("TRIM(LOWER(title))")
            ->get();

        //users
        $values['users'] = User::select("id", "name")
            ->where('id', '>', 1)
            ->where('deleted', 0)
//            ->orderByRaw("TRIM(LOWER(name))")
        ...
        //refers
        $values['refers'] = $this->_apiCore->getRefers();

        return view("pages.be.orders.index", $values);
    }
    ...
    public function setting(Request $request)
    {
        ...
        return view("pages.be.orders.setting", $values);
...
//        echo '<pre>';var_dump($values);die;
        ...
        if (!isset($values['test_giao_hang'])) {
            $row = Setting::where('title', 'test_giao_hang')->first();
            if ($row) {
                $row->update([
                    'value' => '',
                ]);
        ...
        if (!isset($values['test_thanh_toan'])) {
            $row = Setting::where('title', 'test_thanh_toan')->first();
            if ($row) {
                $row->update([
                    'value' => '',
                ]);
        ...
        return redirect('/admin/receipt-settings');
    }
