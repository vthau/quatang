...
$priceDiscount = 0;
if ($item->sale_id) {
    $sale = \App\Model\ProductSale::find($item->sale_id);
    $percentDiscount = $item->sale_discount;
    if ($percentDiscount) {
        $priceDiscount = (int)($priceMain * $percentDiscount / 100);
        $priceOne = $priceMain - $priceDiscount;
    } else {
        $priceOne = $priceMain;
    }
}
                        ...
                        <div id="shopify-section-pr_summary"
                            ...
                            <div class="product_star pr clearfix mb__20">
                                ...
                                <div class="float-left">
                                    <a class="text-bold" href="#product_review">({{count($reviews)}} đánh giá)</a>
                                </div>
                            </div>
...
{{--                            combo--}}
            ...
            <div id="shopify-section-pr_description" class="shopify-section shopify-tabs sp-tabs">
                <ul class="ul_none ul_tabs is-flex fl_center fs__16 des_mb_2 des_style_1 tab_product_info">
                    <li class="tab_title_block ttct active">
                        <a class="db cg truncate pr text-uppercase" href="javascript:void(0)" onclick="jssptab('ttct')">{{$apiCore->getSetting('text_sp_ttct')}}</a>
                    </li>
                    @if (!empty($item->thong_tin_khoa_hoc))
                    <li class="tab_title_block ttkh">
                        <a class="db cg truncate pr text-uppercase" href="javascript:void(0)" onclick="jssptab('ttkh')">{{$apiCore->getSetting('text_sp_ttkh')}}</a>
                    </li>
                    @endif
                    @if (!empty($item->danh_gia_chuyen_gia))
                    <li class="tab_title_block spdg">
                        <a class="db cg truncate pr text-uppercase" href="javascript:void(0)" onclick="jssptab('spdg')">{{$apiCore->getSetting('text_sp_dgcg')}}</a>
                    </li>
                    @endif
                </ul>
                ...
                <div class="row">
                    <div class="col-md-12  p-body">
                        <div class="p-body-content ttct text-center">
                            <div class="row">
                                <div class="col-md-12">
                                    <table class="table table-data__info table_product_info">
                                        ...
                                        @endif
                                        @if (!empty($item->kich_thuoc_vong_hong))
                                            <tr>
                                                <td>{{$apiCore->getSetting('text_sp_ktvh')}}</td>
                                                <td>
                                                    {{$item->kich_thuoc_vong_hong}}
                                                </td>
                                            </tr>
                                        @endif
                                        @if (!empty($item->gioi_tinh))
                                            <tr>
                                                <td>{{$apiCore->getSetting('text_sp_gt')}}</td>
                                                <td>
                                                    {{$item->gioi_tinh}}
                                                </td>
                                            </tr>
                                        @endif
                                        @if (!empty($item->dang_san_pham))
                                            <tr>
                                                <td>{{$apiCore->getSetting('text_sp_dsp')}}</td>
                                                <td>
                                                    {{$item->dang_san_pham}}
                                                </td>
                                            </tr>
                                        @endif
                                        @if (!empty($item->diem_tham_hut))
                                            <tr>
                                                <td>{{$apiCore->getSetting('text_sp_dth')}}</td>
                                                <td>
                                                    {{$item->diem_tham_hut}}
                                                </td>
                                            </tr>
                                        @endif
                                        @if (!empty($item->muc_tham_hut))
                                            <tr>
                                                <td>{{$apiCore->getSetting('text_sp_mth')}}</td>
                                                <td>
                                                    {{$item->muc_tham_hut}}
                                                </td>
                                            </tr>
                                        ...
                                        @endif
                                        @if (!empty($item->don_vi))
                                            <tr>
                                                <td>{{$apiCore->getSetting('text_sp_dv')}}</td>
                                                <td>
                                                    {{$item->don_vi}}
                                                </td>
                                            </tr>
                                        @endif
                                        @if (!empty($item->qui_cach_dong_goi))
                                            <tr>
                                                <td>{{$apiCore->getSetting('text_sp_qcdg')}}</td>
                                                <td>
                                                    {{$item->qui_cach_dong_goi}}
                                                </td>
                                            </tr>
                                        @endif
                            ...
                            @if (!empty($item->thong_tin_chi_tiet_text))
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="xem_them_button" id="ttct_button" onclick="jsspttct()">
                                        <button type="button" class="button">xem thêm</button>
                                    </div>
                                    <div class="xem_them_body hidden" id="ttct_body">
                                        <div style="text-align: left;">
                                            <?php echo $item->thong_tin_chi_tiet;?>
                                        </div>

                                        <div class="xem_them_button" onclick="jsspttct()">
                                            <button type="button" class="button">ẩn thông tin</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <?php if (!empty($item->thong_tin_khoa_hoc)):
                        $ttkh = (array)json_decode($item->thong_tin_khoa_hoc);

                        ?>
                        <div class="p-body-content ttkh hidden">
                            <?php foreach($ttkh as $ite):
                                $arr = array_filter(explode('_', $ite));
                                $i = $apiCore->getItem($arr[0], (int)$arr[1]);
                                if (!$i) {
                                    continue;
                                }
                            ?>
                            <div class="ttkh_item">
                                <div class="ttkh_item_title">
                                    <h4>{{$i->getTitle()}}</h4>
                                </div>
                                <div class="ttkh_item_mtn" id="ttct_mtn_{{$arr[0]}}_{{$arr[1]}}">
                                    @if (!empty($i->mo_ta_ngan))
                                    {{$i->mo_ta_ngan}}
                                    @endif
                                </div>
                                <div class="ttkh_item_button" id="ttct_button_{{$arr[0]}}_{{$arr[1]}}" onclick="jsspttkh('{{$arr[0]}}', '{{$arr[1]}}')">
                                    <button type="button" class="button">xem thêm</button>
                                </div>
                                <div class="ttkh_item_body hidden" id="ttct_body_{{$arr[0]}}_{{$arr[1]}}">
                                    <div>
                                        <?php echo $i->mo_ta;?>
                                    </div>

                                    <div class="ttkh_item_button" onclick="jsspttkh('{{$arr[0]}}', '{{$arr[1]}}')">
                                        <button type="button" class="button">ẩn thông tin</button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <?php if (!empty($item->danh_gia_chuyen_gia)):
                        $dgcg = (array)json_decode($item->danh_gia_chuyen_gia);

                        ?>
                        <div class="p-body-content spdg hidden">
                            <?php foreach($dgcg as $ite):
                            $arr = array_filter(explode('_', $ite));
                            $i = $apiCore->getItem($arr[0], (int)$arr[1]);
                            if (!$i) {
                                continue;
                            ...
                            ?>
                            <div class="dgcg_item">
                                <div class="dgcg_item_star">
                                    <img src="{{url('public/images/star_full.png')}}" />
                                    <img src="{{url('public/images/star_full.png')}}" />
                                    <img src="{{url('public/images/star_full.png')}}" />
                                    @if ($i->star > 3 && $i->star < 4)
                                        <img src="{{url('public/images/star_half.png')}}" />
                                        <img src="{{url('public/images/star_empty.png')}}" />
                                    @elseif ($i->star == 4)
                                        <img src="{{url('public/images/star_full.png')}}" />
                                        <img src="{{url('public/images/star_empty.png')}}" />
                                    @elseif ($i->star > 4 && $i->star < 5)
                                        <img src="{{url('public/images/star_full.png')}}" />
                                        <img src="{{url('public/images/star_half.png')}}" />
                                    @elseif ($i->star == 5)
                                        <img src="{{url('public/images/star_full.png')}}" />
                                ...
                                <div class="dgcg_item_top">
                                    <div class="top_avatar" style="background-image:url('{{$i->getAvatar()}}')"></div>
                                    <div class="top_info">
                                        <div class="top_name">{{$i->name}}</div>
                                        <div class="top_position">{{$i->position}}</div>
                                    </div>
                                </div>

                                <div class="dgcg_item_title">
                                    <h4>{{$i->title}}</h4>
                                </div>
                                <div class="dgcg_item_button" id="dgcg_button_{{$arr[0]}}_{{$arr[1]}}" onclick="jsspdgcg('{{$arr[0]}}', '{{$arr[1]}}')">
                                    <button type="button" class="button">xem thêm</button>
                                </div>
                                <div class="dgcg_item_body hidden" id="dgcg_body_{{$arr[0]}}_{{$arr[1]}}">
                                    <div>
                                        <?php echo $i->mo_ta;?>
                                    </div>

                                    <div class="dgcg_item_button" onclick="jsspdgcg('{{$arr[0]}}', '{{$arr[1]}}')">
                                        <button type="button" class="button">ẩn thông tin</button>
                                    </div>
                                </div>
                            </div>
    ...
    <div class="container container_cat pop_default cat_default mb__60" id="product_review">
        <div class="row al_center fl_center title_10 ">
            <div class="col-auto col-md"><h3 class="dib tc section-title fs__22 text-uppercase">đánh giá sản phẩm</h3></div>
        </div>
    ...
    <div class="container container_cat pop_default cat_default mb__60">
        <div class="row al_center fl_center title_10 ">
            <div class="col-auto col-md"><h3 class="dib tc section-title fs__22 text-uppercase">Sản phẩm bạn đã xem</h3></div>
        </div>
