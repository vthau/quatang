            ...
            <div class="row">
                <div class="col-12 col-md-6 tc order-md-2 order-2 mb__10">
                    <table class="table margin-0">
                        ...
                        @if($cart->getPerson())
                        <tr>
                            <td class="width-25 frm-label">gửi tặng</td>
                            <td class="pr clearfix">
                                <div class="overflow-hidden">
                                    <div>
                                        <div class="text-uppercase">{{$cart->getPerson()->getTitle()}}</div>
                                        @if(!empty($cart->getPerson()->phone))
                                            <div>ĐT: <a href="tel:{{$cart->getPerson()->phone}}">{{$cart->getPerson()->phone}}</a></div>
                                        @endif
                                        <div>
                                            @if(!empty($cart->getPerson()->getRelationship()))
                                                <span class="badge badge-primary text-uppercase">{{$cart->getPerson()->getRelationship()->getTitle()}}</span>
                                            @endif
                                            @if(!empty($cart->getPersonDate()))
                                                <span class="badge badge-primary text-uppercase">{{$cart->getPersonDate()->getTitle() . ' - ' . $cart->getPersonDate()->day . ' / ' . $cart->getPersonDate()->month}}</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                        ...
                        <tr>
                            <td class="frm-label">câu chúc</td>
                            <td>
                                <div><?php echo nl2br($cart->text_wish)?></div>
                            </td>
                        </tr>
                        <tr>
                            <td class="frm-label">mẫu thiệp</td>
                            <td class="clearfix overflow-hidden">
                                @if(count($cards))
                                    @foreach($cards as $photo)
                                    <div class="float-left photo_card_template mr-2 mb-2">
