...
@section('content')
    ...
    <div>
        <div class="fade-in">
            <div class="row">
                <div class="col-md-12">
                    <div class="btn-menu">
                        @if($viewer->isAllowed('product_dgcg_add'))
                            <button class="btn btn-primary btn-sm mb-1" onclick="parent.window.location.href = '{{url('admin/review/add')}}'" >
                                    ...
                                    <div class="form-group">
                                        <div class="col-md-12">
                                            <div class="input-group">
                                                <div class="input-group-btn">
                                                    <div class="btn-group">
                                                        ...
                                                        <div tabindex="-1" aria-hidden="true" role="menu" class="dropdown-menu" x-placement="top-start" style="position: absolute; transform: translate3d(0px, -173px, 0px); top: 0px; left: 0px; will-change: transform;">
                                                            <button onclick="filterBy('title')" type="button" tabindex="0" class="dropdown-item">Tiêu Đề</button>
                                                            <button onclick="filterBy('name')" type="button" tabindex="0" class="dropdown-item">Người Đánh Giá</button>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <input type="hidden" id="filter-by" name="filter" value="{{count($params) && isset($params['filter']) ? $params['filter'] : "title"}}" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <tr id="item-{{$item->id}}">
                                        ...
                                        <td>
                                            <div class="align-right">
                                                @if($viewer->isAllowed('product_dgcg_edit'))
                                                <button class="btn btn-info btn-sm mb-1"
                                                        title="Sửa" data-original-title="Sửa"
                                                        onclick="gotoPage('{{url('admin/review/add?id=' . $item->id)}}')"
                                                >
                                                    <i class="fa fa-edit"></i>
                                                </button>
                                                @endif

                                                @if($viewer->isAllowed('product_dgcg_delete'))
    ...
    <script type="text/javascript" src="{{url('public/js/be/reviews.js')}}"></script>
@stop
